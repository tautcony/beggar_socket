...
void usb_set_configuration()
{
    ...
    if (Setup.wValueL == 1)
    {
        ...
        OutEpState = 0x00;
#ifdef EN_EP1IN
        usb_write_reg(INDEX, 1);               // 选中端点1
        usb_write_reg(INCSR2, INMODEIN);       // 选中输入端点1，设置为批量端点
        usb_write_reg(INMAXP, EP1IN_SIZE / 8); // 端点包长64
#endif
#ifdef EN_EP2IN
...
#ifdef EN_EP1OUT
        usb_write_reg(INDEX, 1);                 // 选中端点1
        usb_write_reg(INCSR2, INMODEOUT);        // 选中输出端点1，设置为批量端点
        usb_write_reg(OUTMAXP, EP1OUT_SIZE / 8); // 端点包长64
#endif
#ifdef EN_EP2OUT
        usb_write_reg(INDEX, 2);
