...
import { gbc_read, gbc_write, getPackage, getResult, ram_read, rom_erase_sector, rom_read, sendPackage, setSignals } from '@/protocol';
import { readProtocolPayload } from '@/protocol/beggar_socket/packet-read';

describe('Protocol transport abstraction', () => {
  it('protocol utilities send and read through Transport', async () => {
    ...
    await expect(sendPackage(transport, new Uint8Array([1, 2, 3]), 10)).resolves.toBe(true);
    await expect(getResult(transport, 20)).resolves.toBe(true);
    await expect(setSignals(transport, { dataTerminalReady: true })).resolves.toBeUndefined();
  ...
  it('protocol utilities normalize packet result semantics', async () => {
    const transport: Transport = {
    ...
    await expect(getPackage(transport, 2, 30, 'default')).resolves.toEqual({ data: new Uint8Array([0x00]) });
    await expect(getResult(transport, 30)).resolves.toBe(false);
  ...
  it('canonical packet-read failures expose stable protocol error codes', async () => {
    ...
    await expect(ram_read(timeoutTransport, 4, 0x20)).rejects.toMatchObject({
      code: 'PACKET_TIMEOUT',
    });
    ...
    await expect(readProtocolPayload(shortTransport, 'GBC read', 2, 0x10)).rejects.toMatchObject({
