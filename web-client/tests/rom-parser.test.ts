...
describe('rom-parser', () => {
  describe('GBA ROM解析', () => {
    // 创建一个有效的GBA ROM头部
    function createGBARomHeader(): Uint8Array {
      const header = new Uint8Array(0x200); // 512字节足够测试
  ...
  describe('GB/GBC ROM解析', () => {
    function createGBRomHeader(isGBC = false): Uint8Array {
  ...
  describe('Logo验证', () => {
    ...
    it('Logo数据应该是只读的', () => {
      // 这个测试确保我们不会意外修改logo数据
      const originalGBALogo = new Uint8Array(GBA_NINTENDO_LOGO);
      const originalGBLogo = new Uint8Array(GB_NINTENDO_LOGO);

      expect(GBA_NINTENDO_LOGO).toEqual(originalGBALogo);
      expect(GB_NINTENDO_LOGO).toEqual(originalGBLogo);
    });
  });
});
