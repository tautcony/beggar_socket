...
`GBAMultiMenuView.vue` 是项目中最大的单文件组件（1138 行），承载了 5 个以上的独立职责：游戏 ROM 列表管理与配置编辑、存档文件处理、背景图片上传/裁剪/预览、构建配置选项、ROM 构建与结果下载。单文件内多职责混合导致定位具体逻辑困难，修改一个功能可能意外影响其他功能，且单元测试极难编写。
